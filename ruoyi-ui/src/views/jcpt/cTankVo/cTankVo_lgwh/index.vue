<template>
    ...
    <div class="app-container">
        ...
        <el-table v-loading="loading" :data="dataList" @selection-change="handleSelectionChange">
            ...
            <el-table-column label="区间下限" sortable align="center" key="secFlrLmt" prop="secFlrLmt"
                :show-overflow-tooltip="true" v-if="columns[3].visible" />
            <el-table-column label="区间上限" sortable align="center" key="secUpLmt" prop="secUpLmt"
        ...
        <el-dialog :title="title" :visible.sync="open" width="800px" align append-to-body>
            <el-form ref="form" :model="form" :rules="rules" label-width="120px">
                <el-row>
                    ...
                    <el-col :span="10">
                        <el-form-item label="区间范围编号:" prop="secId">
                            <el-input :disabled="form.id" v-model="form.secId" placeholder="区间范围编号" maxlength="11" />
                        </el-form-item>
                    </el-col>
                </el-row>
                ...
                <el-row>
                    <el-col :span="10">
                        <el-form-item label="区间下限:" prop="secFlrLmt">
                            <el-input type="number" v-model="form.secFlrLmt" placeholder="请输入区间下限" maxlength="30" />
                        </el-form-item>
                    </el-col>
                    <el-col :span="4" style="height: 10px;">
                    </el-col>
                    <el-col :span="10">
                        <el-form-item label="区间上限:" prop="secUpLmt">
                            <el-input type="number" v-model="form.secUpLmt" placeholder="请输入区间上限" maxlength="11" />
                        </el-form-item>
                    </el-col>
                </el-row>
            </el-form>
...
export default {
    ...
    data() {
        return {
            ...
            upload: {
                ...
                // 上传的地址
                url: process.env.VUE_APP_BASE_API + "mes/stdSec/import"
            },
            // 查询参数
            queryParams: {
                "id": undefined,
            ...
            columns: [
                ...
                { key: 3, label: `区间下限`, visible: true },
                { key: 4, label: `区间上限`, visible: true },
            ],
            //罐数据
            tankList: [],
        };
    ...
    methods: {
        ...
        getList() {
            this.loading = true;
            this.$api.i_stdSecList(this.queryParams).then(response => {
            ...
            this.$refs["form"].validate(valid => {
                if (valid) {
                    if (this.form.id != undefined) {
                        this.$api.i_stdSecSU({
                            "id": this.form.id,
                            "secFlrLmt": this.form.secFlrLmt,
                            "secId": this.form.secId,
                            "secUpLmt": this.form.secUpLmt,
                        ...
                        this.$api.i_stdSecSU(this.form).then(response => {
        ...
        handleUpdate(row) {
            ...
            this.$api.i_stdSecById({ id: userId }).then(response => {
        ...
        handleDelete(row) {
            ...
            this.$modal.confirm('是否确认删除"' + userIds + '"的数据项？').then(() => {
                this.$api.i_stdSecDel({ id: userIds })
            }).then(() => {
        ...
        handleBatchDelete() {
            this.$modal.confirm('是否确认批量删除"' + this.queryParams.id + '"的数据项？').then(() => {
                return this.$api.i_stdSecdeleteByTankId({ tankId: this.queryParams.id })
            }).then(() => {
                this.getList();
                this.$modal.msgSuccess("删除成功");
            }).catch(() => { });
