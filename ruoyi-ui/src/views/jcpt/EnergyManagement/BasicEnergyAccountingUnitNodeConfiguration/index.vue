<template>
    ...
    <div class="jcc-con">
        <div class="jcc-left">
            <div class="jcc-top">
                <el-card class="box-card">
                    <div style="margin-bottom: 10px;">基本能源核算单元节点配置</div>
                    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch"
                        label-width="50">
                        <el-form-item label="基本核算单元:" prop="unitType">
                            <el-select size="mini" v-model="queryParams.unitType" style="width: 140px;" placeholder="基本核算单元"
                                clearable>
                    ...
                    <div class="">
                        <el-table border v-loading="loading" :data="sideLineList" height="230"
                            ...
                            <el-table-column label="核算单元名称" align="center" key="name" prop="name" />
                        </el-table>
        ...
        <el-card class="jcc-mon">
            <el-form :model="queryParams" ref="queryFormLeft" size="small" :inline="true" v-show="showSearch"
                label-width="50">
                <el-form-item label="区域:" prop="nodeAreaType">
                    <el-select v-model="queryParams.nodeAreaType" style="width: 140px;" placeholder="区域" clearable>
                        <el-option v-for="dict in OwningRegion" :key="dict.value" :label="dict.label" :value="dict.value" />
                    </el-select>
                </el-form-item>
                <el-form-item label="节点类型:" prop="type">
                    <el-select v-model="queryParams.type" style="width: 140px;" placeholder="节点类型" clearable>
                        <el-option v-for="dict in nodeTypeList" :key="dict.value" :label="dict.label" :value="dict.value" />
                    </el-select>
                </el-form-item>
                <el-form-item label="节点名称:" prop="nodeName">
                    <el-input v-model="queryParams.nodeName" style="width: 130px;" placeholder="节点名称" clearable
                        @keyup.enter.native="handleQueryLeft" />
                </el-form-item>
                <el-form-item label="介质:" prop="mediumId">
                    <el-select v-model="queryParams.mediumId" style="width: 140px;" placeholder="介质" clearable>
...
<script>
export default {
    name: "BasicEnergyAccountingUnitNodeConfiguration",
    data() {
        return {
            SuperiorMaterial: [],//介质
            OwningRegion: [],//所属区域
    ...
    methods: {
        ...
        leftMod() {
            ...
            this.$api.e_basicEnergynodeSU(
                {
                    nodeId: this.idsL[0],
                    basicId: this.currentSideLine.id,
                }
            ).then((res) => {
                if (res.code == 200) {
        ...
        rightMod() {
            //左 转 右
            this.$api.e_basicEnergynodeDel({ id: this.idsR[0] }).then((res) => {
                if (res.code == 200) {
                    this.handleTopItemSelected(this.currentSideLine)
        ...
        querySideLine(param) {
            ...
            this.$api.e_basicCalculateUnitList(param).then((res) => {
                if (res.code == 200) {
                    // 源节点列表
        ...
        handleTopItemSelected(item) {
            ...
            //右
            const promise2 = this.$api.e_basicEnergynodeList(
                this.queryParams
            )
