<template>
  ...
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" label-width="130px" @submit.native.prevent>
      <el-form-item label="巡检计划开始时间:" prop="requisitionDate">
        <el-date-picker v-model="queryParams.requisitionDate" type="datetime" placeholder="巡检计划开始时间" style="width: 100%"
          clearable>
        </el-date-picker>
      </el-form-item>
      <el-form-item label="区域:" prop="department">
        <el-select size="mini" v-model="queryParams.department" placeholder="区域" clearable>
          <el-option v-for="dict in RequisitionDepartment" :key="dict.value" :label="dict.label" :value="dict.value" />
        </el-select>
      </el-form-item>
      <el-form-item label="任务状态:" prop="status">
        <el-select size="mini" v-model="queryParams.status" placeholder="任务状态" clearable>
          <el-option v-for="dict in AuditStatus" :key="dict.value" :label="dict.label" :value="dict.value" />
        </el-select>
      </el-form-item>
      <el-form-item label="设备名称:" prop="name">
        <el-input type="text" v-model="queryParams.name" maxlength="20" placeholder="设备名称" clearable></el-input>
      </el-form-item>
      <el-form-item>
    ...
    <el-table v-loading="loading" :data="dataList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="巡检计划名称" align="center" key="name" prop="name" :show-overflow-tooltip="true">
      ...
      <el-table-column label="巡检计划编号" align="center" key="num" prop="num" :show-overflow-tooltip="true" />
      <el-table-column label="所属区域" align="center" key="Associatedorder" prop="Associatedorder"
        :show-overflow-tooltip="true" />
      <el-table-column label="涉及设备" align="center" key="Equipmentinvolved" prop="Equipmentinvolved"
        :show-overflow-tooltip="true" />
      ...
      <el-table-column label="巡检内容" align="center" key="Inspectioncontent" prop="Inspectioncontent"
        :show-overflow-tooltip="true" />
      <el-table-column label="计划开始时间" align="center" key="Purchasetime" prop="Purchasetime"
        :show-overflow-tooltip="true" />
      <el-table-column label="计划结束时间" align="center" key="Plannedtime" prop="Plannedtime" :show-overflow-tooltip="true" />
      <el-table-column label="执行规则" align="center" key="Actualtime" prop="Actualtime" :show-overflow-tooltip="true" />
      <el-table-column label="计划提报人" align="center" key="department" prop="department" :show-overflow-tooltip="true">
      </el-table-column>
      <el-table-column label="计划审批人" align="center" key="user" prop="user" :show-overflow-tooltip="true" />
      <el-table-column label="是否循环" align="center" key="useFlag" prop="useFlag" :show-overflow-tooltip="true">
        <template slot-scope="scope">
          <el-checkbox disabled true-label="1" false-label="0" v-model="scope.row.useFlag"></el-checkbox>
        </template>
      </el-table-column>
      <el-table-column label="巡检规章" align="center">
        <template slot-scope="scope">
      ...
      <el-row v-if="filesDialog.type == 'files'">
        <el-col :span="12" v-for="(file, index) in filesDialog.data" :key="index" style="margin-bottom: 10px;">
          <el-row class="flex align-center">
            ...
            <el-col :span="6">
              <svg-icon v-if="file.type != 'ppt'" class-name="svg-icon medium cs mr10" icon-class="look"
...
export default {
  ...
  methods: {
    ...
    handleAdd() {
      this.$router.push({
        path: "/equipment/inspection/ition_jh/addOrEdit",
        query: {
          id: "",
        },
      });
    ...
    handleToDetail(id) {
      this.$router.push({
        path: "/equipment/inspection/ition_rw/ition_rwdetail",
        query: {
          id: id,
        },
      });
