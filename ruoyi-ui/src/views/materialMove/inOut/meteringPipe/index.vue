<template>
    ...
    <div class="app-container">
        <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="50px">
            <el-form-item label="状态:" prop="status">
                <el-select v-model="queryParams.status" placeholder="请选择状态" clearable>
                    <el-option v-for="dict in DocumentType" :key="dict.value" :label="dict.label" :value="dict.value" />
                </el-select>
            </el-form-item>
        ...
        <el-table ref="multipleTable" v-if="dataList && dataList.length > 0" border v-loading="loading" :data="dataList"
            @selection-change="handleSelectionChange" :expand-row-keys="expands">
    ...
    methods: {
        ...
        handleClose(row) {
            ...
            this.$modal.confirm('是否确认关闭"' + userIds + '"的数据项？').then(() => {
                this.$api.i_meteringPipeClose({ id: userIds })
            }).then(() => {
                this.getList();
            ...
            this.$refs["form"].validate(valid => {
                if (valid) {
                    if (this.form.id != undefined) {
                        ...
                        this.$api.i_meteringPipeAdd(this.form).then(response => {
        ...
        handleDelete(row) {
            ...
            this.$modal.confirm('是否确认删除"' + userIds + '"的数据项？').then(() => {
                this.$api.i_meteringPipeDelete({ id: userIds })
            }).then(() => {
                this.getList();
