<template>
    ...
    <div class="app-container">
        ...
        <el-table ref="multipleTable" v-if="dataList && dataList.length > 0" border v-loading="loading" :data="dataList"
            ...
            <el-table-column label="备注" align="center" key="memo" prop="memo" width="120" />
        </el-table>
        <!--    物料组分    -->
...
export default {
    ...
    methods: {
        ...
        getList() {
            this.loading = true;
            //主数据
            this.$api.i_automobileIncomingMeasurementSheetSelece({
        ...
        handleClose(row) {
            ...
            this.$modal.confirm('是否确认删除"' + userIds + '"的数据项？').then(() => {
                return this.$api.i_automobileIncomingMeasurementSheetClose({ id: userIds })
            }).then(() => {
                this.getList();
            ...
            this.$refs["form"].validate(valid => {
                if (valid) {
                    if (this.form.id != undefined) {
                        this.$api.i_automobileIncomingMeasurementSheetAdd({
                        ...
                        this.$api.i_automobileIncomingMeasurementSheetAdd(this.form).then(response => {
        ...
        handleDelete(row) {
            const userIds = row.id || this.ids[0];
            this.$modal.confirm('是否确认删除"' + userIds + '"的数据项？').then(() => {
                this.$api.i_automobileIncomingMeasurementSheetDelete({ id: userIds })
            }).then(() => {
                this.getList();
