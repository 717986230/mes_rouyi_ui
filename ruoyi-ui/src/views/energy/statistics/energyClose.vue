<template>
    ...
    <div class="app-container">
        <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="100px">
            ...
            <el-form-item label="MES工厂:" prop="unitTypeId">
                <el-select style="width: 100px;" v-model="queryParams.unitTypeId" placeholder="MES工厂" clearable>
                    <el-option v-for="dict in zzType" :key="dict.value" :label="dict.label" :value="dict.value" />
                </el-select>
            </el-form-item>
            <el-form-item label="能源:" prop="unitName">
                <el-select style="width: 100px;" v-model="queryParams.nodeAreaId" placeholder="能源" clearable>
                    <el-option v-for="dict in FormJQtype" :key="dict.value" :label="dict.label" :value="dict.value" />
                </el-select>
            </el-form-item>
            <el-form-item label="数据类型:" prop="nodeAreaId">
                <el-select style="width: 100px;" v-model="queryParams.nodeAreaId" placeholder="数据类型" clearable>
                    <el-option v-for="dict in FormJQtype" :key="dict.value" :label="dict.label" :value="dict.value" />
                </el-select>
            </el-form-item>
            <el-form-item>
        ...
        <el-table v-loading="loading" border :data="userList" @selection-change="handleSelectionChange">
            ...
            <el-table-column fixed="left" width="130" label="核算单元名称" align="center" sortable key="unitNum" prop="unitNum"
                v-if="columns[0].visible" />
            <el-table-column label="能源" align="center" sortable key="unitCode" prop="unitCode" v-if="columns[1].visible"
                :show-overflow-tooltip="true" />
            <el-table-column label="数据类型" align="center" sortable key="unitName" prop="unitName" v-if="columns[2].visible"
                :show-overflow-tooltip="true" />
            <el-table-column label="结束时间" align="center" sortable key="unitAlias" prop="unitAlias" v-if="columns[3].visible"
                :show-overflow-tooltip="true" />
            <el-table-column label="原始量" align="center" sortable key="typeName" prop="typeName" v-if="columns[4].visible"
                :show-overflow-tooltip="true" />
            <el-table-column label="校正量" align="center" sortable key="nodeAreaName" prop="nodeAreaName"
                v-if="columns[5].visible" :show-overflow-tooltip="true" />
            <el-table-column label="管网平衡量" align="center" sortable key="upperUnitName" prop="upperUnitName" width="120"
                v-if="columns[6].visible" />
            <el-table-column label="核算实际量" align="center" sortable key="capacity" prop="capacity" width="130"
                v-if="columns[7].visible" />
            <el-table-column label="核算结算量" align="center" sortable key="phonenumber" prop="phonenumber" width="120"
                v-if="columns[8].visible" />
            <el-table-column label="数据状态" align="center" sortable key="phonenumber" prop="phonenumber" width="120"
                v-if="columns[8].visible" />
    ...
    data() {
        return {
            ...
            columns: [
                ...
                { key: 1, label: `能源`, visible: true },
                { key: 2, label: `数据类型`, visible: true },
                { key: 3, label: `结束时间`, visible: true },
                { key: 4, label: `原始量`, visible: true },
                { key: 5, label: `校正量`, visible: true },
                { key: 6, label: `管网平衡量`, visible: true },
                { key: 7, label: `核算实际量`, visible: true },
    ...
    methods: {
        ...
        handleExport() {
            this.download('mes/unit/export', {
                ...this.queryParams
            }, `能源封账/完工确认_${new Date().getTime()}.xlsx`)
        },
        /** 导入按钮操作 */
        handleImport() {
