<template>
    ...
    <div class="app-container">
        ...
        <el-table ref="multipleTable" v-if="dataList && dataList.length > 0" border v-loading="loading" :data="dataList"
            ...
            <el-table-column label="单据总量" align="center" key="Amount" prop="Amount" width="120" />
            <el-table-column label="确认总量" align="center" key="ConAmount" prop="ConAmount" width="120" />
            <el-table-column label="单据状态" align="center" key="status" prop="status" width="120" />
            <el-table-column label="物料组分" align="center" key="CmtTime" prop="CmtTime" width="120">
                <template slot-scope="scope">
                    <el-button type="text" @click="handleMaterial">查看</el-button>
                </template>
            </el-table-column>
            <el-table-column label="辅计量" align="center" key="CrtUserName" prop="CrtUserName" width="120">
                <template slot-scope="scope">
            ...
            <el-table-column label="备注" align="center" key="des" prop="des" width="120" />
        </el-table>
        <!--    物料组分    -->
            ...
            <el-form ref="form" :model="form" :rules="rules" label-width="90px">
                ...
                <el-row type="flex" justify="center">
                    <el-col :span="6" class="flex align-center justify-center">
                        ...
                        <el-button type="success" size="mini" @click="handleComplete">完成</el-button>
                    </el-col>
                </el-row>
            </el-form>
            <el-tabs v-model="activeName" @tab-click="handleClick">
                <el-tab-pane label="计量记录" name="1">
                    <el-button type="success" size="mini" @click="TableMeasurement">表计量</el-button>
                    <el-button type="success" size="mini" @click="TrackscaleMeasurement">轨道衡计量</el-button>
                    <el-button type="success" size="mini" @click="TrackScaleImport">轨道衡导入</el-button>
                    <el-button type="success" size="mini" @click="TankMetering">罐计量</el-button>
                    <el-button type="success" size="mini" @click="ShipMeasurement">船计量</el-button>
                    <el-button type="danger" size="mini" @click="handleDel">删除</el-button>
                    <el-button type="primary" size="mini" @click="handleMaintenance"> 维护</el-button>
...
<script>
export default {
    name: "AutomobileIncomingMeasurementSheet",
    components: {},
    data() {
        return {
            banCi: [{
    ...
    methods: {
        ...
        getList() {
            this.loading = true;
            //子数据
            this.$api.i_automobileFactoryMeasurementSheetSelece({
            ...
            this.$refs["form"].validate(valid => {
                if (valid) {
                    if (this.form.id != undefined) {
                        this.$api.i_automobileFactoryMeasurementSheetAdd({ "mainId": this.form.mainId }).then(response => {
                            this.$modal.msgSuccess("修改成功");
                            this.open = false;
                            this.getList();
                        });
                    } else {
                        this.$api.i_automobileFactoryMeasurementSheetAdd(this.form).then(response => {
        ...
        handleClose(row) {
            ...
            this.$modal.confirm('是否确认删除"' + userIds + '"的数据项？').then(() => {
                return this.$api.i_automobileFactoryMeasurementSheetClose({ id: userIds })
            }).then(() => {
                this.getList();
        ...
        handleDelete(row) {
            const userIds = row.id || this.ids[0];
            this.$modal.confirm('是否确认删除"' + userIds + '"的数据项？').then(() => {
                return this.$api.i_automobileFactoryMeasurementSheetDelete({ id: userIds })
            }).then(() => {
                this.getList();
