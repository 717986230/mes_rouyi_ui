<template>
    ...
    <div class="app-container">
        ...
        <el-table v-loading="loading" border :data="userList" @selection-change="handleSelectionChange"
                ...
                <template slot-scope="props">
                    <div style="padding: 0px 50px">
                        <el-table :data="props.row.childList">
                            <el-table-column label="测点名称" width="200" align="center" type="index" key="measurementName"
                                             prop="measurementName"/>
                            <el-table-column label="原始值" align="center" width="200" key="originalValue"
                                             prop="originalValue" />
                            <el-table-column label="确认值" align="center" width="200" key="confirmedValue"
                                             prop="confirmedValue" />
                        </el-table>
                    </div>
                </template>
            ...
            <el-table-column label="节点类型" align="center" sortable key="nodeType" prop="nodeType" v-if="columns[2].visible"
                :show-overflow-tooltip="true" />
            <el-table-column label="开始时间" align="center" sortable key="startTime" prop="startTime" v-if="columns[3].visible"
                :show-overflow-tooltip="true">
                <template slot-scope="scope">
                    {{ dayjs(scope.row.startTime).format("YYYY-MM-DD HH:mm:ss") }}
                </template>
            </el-table-column>
            <el-table-column label="结束时间" align="center" sortable key="endTime" prop="endTime" v-if="columns[4].visible"
                :show-overflow-tooltip="true">
                <template slot-scope="scope">
                    {{ dayjs(scope.row.endTime).format("YYYY-MM-DD HH:mm:ss") }}
                </template>
            </el-table-column>
            <el-table-column label="原始量" align="center" sortable key="originalQuantity" prop="originalQuantity" v-if="columns[5].visible"
                :show-overflow-tooltip="true">
                <template slot-scope="scope">
                    <div v-if="isShow">
                        {{ scope.row.originalQuantity }}
                    </div>
                </template>
            </el-table-column>
            <el-table-column label="确认量" align="center" sortable key="confirmedQuantity" prop="confirmedQuantity"
...
export default {
    ...
    methods: {
        ...
        getList() {
            this.loading = true;
            this.$api.i_nodeConfirmList(this.queryParams).then(response => {
