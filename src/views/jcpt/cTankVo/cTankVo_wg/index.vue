<template>
  ...
  <div class="app-container">
      ...
      <el-form-item>
        <el-form-item label="" prop="tankId" style="margin-left: 300px;">
          <el-select filterable v-model="queryParams.tankId" clearable>
            <el-option v-for="tank in tankList" :key="tank.value" :label="tank.value" :value="tank.value" />
          </el-select>
        </el-form-item>
    ...
    <el-table v-loading="loading" :data="dataList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="罐编号" sortable align="center" prop="tankId" v-if="columns[0].visible" />
      <el-table-column label="罐名称" sortable align="center" prop="tankName" v-if="columns[1].visible"
        :show-overflow-tooltip="true" />
      <el-table-column label="罐高" sortable align="center" prop="hgt" v-if="columns[2].visible"
        :show-overflow-tooltip="true" />
      <el-table-column label="罐容" sortable align="center" prop="cuba" v-if="columns[3].visible"
        :show-overflow-tooltip="true" />
    ...
    <el-dialog :title="title" :visible.sync="open" width="600px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="100px">
        <el-row type="flex" justify="center">
          <el-col :span="24">
            <el-form-item label="罐名称:" prop="tankId">
              <el-select v-model="form.tankId" placeholder="罐名称" clearable style="width: 100%;">
        ...
        <el-row type="flex" justify="center">
          <el-col :span="24">
            <el-form-item label="罐高:" prop="hgt">
              <el-input v-model="form.hgt" type="number" placeholder="请输入罐高" maxlength="30" />
            </el-form-item>
          </el-col>
        </el-row>
        <el-row type="flex" justify="center">
          <el-col :span="24">
            <el-form-item label="罐容:" prop="cuba">
              <el-input v-model="form.cuba" type="number" placeholder="罐容" maxlength="30" />
            </el-form-item>
          </el-col>
        </el-row>
      </el-form>
...
export default {
  ...
  data() {
    return {
      ...
      upload: {
        ...
        // 上传的地址
        url: process.env.VUE_APP_BASE_API + "mes/cieCuba/import"
      },
      // 查询参数
      queryParams: {
        "hgt": "",
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      this.$api.i_cieCubaList(this.queryParams).then(res => {
    ...
    //获取罐列表
    getTankList() {
      this.$api.i_cieCubatankList().then((res) => {
        if (res.code == 200) {
    ...
    handleUpdate(row) {
      ...
      this.$api.i_cieCubaByid({ id: userId }).then(response => {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != undefined) {
            ...
            this.$api.i_cieCubaSU(this.form).then(response => {
    ...
    handleDeleteTs() {
      this.$modal.confirm('是否确认批量删除"' + this.queryParams.tankId + '"的数据项？').then(() => {
        return this.$api.i_glbCubaDelByTankId({ tankId: this.queryParams.tankId })
      }).then(() => {
        this.getList();
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除"' + userIds[0] + '"的数据项？').then(() => {
        return this.$api.i_cieCubaDel({ id: userIds[0] })
      }).then(() => {
        this.getList();
        this.$modal.msgSuccess("删除成功");
      }).catch(() => { });
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('mes/cieCuba/export', {
        ...this.queryParams
