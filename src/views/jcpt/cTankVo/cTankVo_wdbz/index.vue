    ...
    <div class="app-container">
        <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="80px">
            <el-form-item label="物料:" prop="artcSt">
                <el-select v-model="queryParams.mtrlName" placeholder="请选择物料" clearable>
                    <el-option v-for="mtr in mtrList" :key="mtr.value" :label="mtr.name" :value="mtr.name" />
                </el-select>
            </el-form-item>
            <el-form-item>
        ...
        <el-table v-loading="loading" :data="dataList" @selection-change="handleSelectionChange">
            ...
            <el-table-column label="物料" sortable align="center" key="mtrlName" prop="mtrlName" v-if="columns[2].visible"
                :show-overflow-tooltip="true" />
            <el-table-column label="提及温度系数（温度）" sortable align="center" key="cubaTempCofe" prop="cubaTempCofe"
...
export default {
    ...
    data() {
        return {
            ...
            upload: {
                ...
                // 上传的地址
                url: process.env.VUE_APP_BASE_API + "/mes/temperatureStandard/import"
            },
            // 查询参数
            queryParams: {
                mtrlName: ""
    ...
    methods: {
        ...
        getList() {
            this.loading = true;
            this.$api.i_temperatureStandardList(this.queryParams).then((res) => {
                if (res.code == 200) {
        ...
        submitForm: function () {
            this.$refs["form"].validate(valid => {
                if (valid) {
                    ...
                    this.$api.i_temperatureStandardSU(this.form).then((res) => {
                        if (res.code == 200) {
        ...
        handleDelete(row) {
            ...
            this.$modal.confirm('是否确认删除"' + userIds + '"的数据项？').then(() => {
                this.$api.i_temperatureStandardDel({ id: userIds })
            }).then(() => {
        ...
        /** 导出按钮操作 */
        handleExport() {
            this.download('mes/temperatureStandard/export', {
                ...this.queryParams
            }, `温度标准密度表_${new Date().getTime()}.xlsx`)
        },
        /** 导入按钮操作 */
        handleImport() {
